// sdxl_vae_decoder.cpp - SDXL VAE decoder implementation
...
SDXLVaeDecoder::SDXLVaeDecoder(
    std::map<ComponentType, std::unique_ptr<OnnxModel>>& components)
    : components_(components) {}

std::vector<uint8_t> SDXLVaeDecoder::decode(
    const std::vector<float>& latents,
    int height, int width) {

    std::cout << "Decoding latents to image (SDXL)..." << std::endl;
    ...
    const float scaling_factor = 0.13025f;
...
std::vector<uint8_t> SDXLVaeDecoder::convert_image_f32(
...
std::vector<uint8_t> SDXLVaeDecoder::convert_image_f16(
